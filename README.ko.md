...
일본에 와서 일본인 친구들과 지내며 한국 뷰티에 관심을 가진 사람이 생각보다 많다는 것을 알게 되었습니다. 지인에게 한국 여행 전 피부 시술과 스킨케어를 추천해 달라는 부탁을 받으면서, 공식 사이트·블로그·커뮤니티·유튜브에 정보는 많지만 사용자가 직접 여러 출처를 찾아 비교하고 연결해야 한다는 점에 주목했습니다.

특히 성인식, 내정식, 결혼식처럼 중요한 일정을 앞둔 사용자는 목적과 남은 시간에 맞는 선택지를 확인하고 싶어 합니다. 이 프로젝트는 정보가 없어서가 아니라 정보가 연결되어 있지 않아서 생기는 탐색 부담을 줄이기 위해, 시술·제품 정보를 목적, 일정, 가격, 주의사항, 출처, 최종 확인일과 함께 연결하는 일본어 우선 위키를 만드는 데서 시작했습니다.

사용자가 직접 말한 요구사항뿐 아니라 실제 탐색 과정에서 드러나는 숨은 요구를 제품 요구사항으로 정의하고, 리뷰 원문을 무단 복사하지 않는 출처·권리·검수 정책으로 구현합니다.
