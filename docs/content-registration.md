...
공개 API는 저장 항목 전체를 전달하지 않고, 공개 콘텐츠 스키마에 정의된 허용 필드만 반환한다. `publicationApproval`, `reviewEvidence`의 내부 승인 정보, 감사 메모와 향후 추가될 내부 필드는 공개 응답에서 제외한다.

관리자 콘텐츠 목록에는 동일한 공개 검증을 미리 실행한 `publicationReadiness`가 표시된다. 이는 검수 안내용 계산 결과일 뿐 저장 대상이 아니며, 최종 공개 요청은 API가 다시 검증한다.

EventBridge로 매일 실행되는 `maintenance-job`은 공개 콘텐츠의 `lastVerifiedAt`를 확인한다. 확인일이 180일보다 오래되었거나 잘못된 경우 로그에 콘텐츠 ID를 남겨 재검수 대상으로 삼으며, 콘텐츠를 자동으로 공개·삭제·수정하지 않는다.

관리자 콘텐츠 목록에서도 같은 정책으로 `정보 신선도 양호` 또는 `재검수 필요`를 표시한다. 신선도 상태와 공개 준비도 계산 결과는 안내용 메타데이터이며, 콘텐츠 저장 시 제외된다.
