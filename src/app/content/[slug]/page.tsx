    ...
    <article className="mx-auto max-w-5xl px-5 py-12 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_0.72fr]">
        ...
        <aside className="h-fit rounded-2xl border border-line bg-white/65 p-5 lg:sticky lg:top-6">
          ...
          {content.reviewEvidence && <div className="mt-6 border-t border-line pt-5"><p className="text-xs text-ink/50">レビュー集計（参考）</p><p className="mt-2 text-sm leading-6 text-ink/75">{content.reviewEvidence.summary}</p><dl className="mt-3 space-y-2 text-xs text-ink/60"><div className="flex justify-between gap-3"><dt>プラットフォーム</dt><dd>{content.reviewEvidence.platform}</dd></div><div className="flex justify-between gap-3"><dt>確認した標本</dt><dd>{content.reviewEvidence.sampleCount}件 / 全{content.reviewEvidence.reviewCountAtCollection}件</dd></div><div className="flex justify-between gap-3"><dt>独立出典</dt><dd>{content.reviewEvidence.independentSourceCount}件</dd></div><div className="flex justify-between gap-3"><dt>集計日</dt><dd>{content.reviewEvidence.collectedAt}</dd></div></dl><p className="mt-3 text-xs leading-5 text-ink/55">レビューは個人の経験に基づく参考情報であり、効果・安全性・適合性を保証するものではありません。</p>{content.reviewEvidence.sourceUrls.map((url) => <a className="mt-2 block break-all text-xs underline decoration-ink/20 underline-offset-4" href={url} key={url} rel="noreferrer">レビュー根拠を見る</a>)}</div>}
